...
module.exports=function createTreasureRaid(deps){
  ...
  const rooms=new Map();
  const MAX_ROUNDS=5,MIN_ENTRY=10000,MAX_ENTRY=100000000,MAX_MULT=100;
  const OUTCOMES={
  ...
  function normalizeEntry(v){const n=Math.floor(Number(v));if(!Number.isSafeInteger(n)||n<MIN_ENTRY||n>MAX_ENTRY||n%10000!==0)throw new Error(`참가금은 ${formatMoney(MIN_ENTRY)}G~${formatMoney(MAX_ENTRY)}G, 10,000G 단위로 설정해줘.`);return n}
  ...
  function playerPublic(p){return {userId:p.userId,nickname:p.nickname,avatar:p.avatar,ready:!!p.ready,status:p.status,round:p.round,bank:p.bank,entry:p.entry,lastChest:p.lastChest,lastOutcome:p.lastOutcome,payout:p.payout||0,...safeProfile(p.userId)}}
  function roomPublic(r,selfId){return {id:r.id,name:r.name,entry:r.entry,maxPlayers:r.maxPlayers,hostId:r.hostId,phase:r.phase,maxRounds:MAX_ROUNDS,createdAt:r.createdAt,updatedAt:r.updatedAt,version:r.version,selfId:Number(selfId),players:r.players.map(playerPublic)}}
  function summary(r){return {id:r.id,name:r.name,entry:r.entry,maxPlayers:r.maxPlayers,players:r.players.length,phase:r.phase,updatedAt:r.updatedAt}}
  ...
  function settlePlayer(r,p,reason='탈출 정산'){
    ...
    const payout=Math.max(0,Math.min(r.entry*MAX_MULT,Math.floor(Number(p.bank||0))));
    ...
    p.payout=payout;p.status=p.round>=MAX_ROUNDS?'complete':'escaped';p.ready=false;touch(r);maybeComplete(r);pushRefresh();return payout;
  }
  function refundWaiting(r,p,reason='대기방 퇴장 환급'){
    const amount=Math.max(0,Number(p.entry||r.entry));escrowDelete(r.id,p.userId);if(amount>0)walletChange(p.userId,amount,'treasure_raid_refund',`${r.name} · ${reason}`);return amount
  }
  function removePlayer(r,p){r.players=r.players.filter(x=>Number(x.userId)!==Number(p.userId));if(!r.players.length){rooms.delete(r.id);return}if(Number(r.hostId)===Number(p.userId))r.hostId=r.players[0].userId;touch(r)}
  function createPlayer(u,entry){return {userId:u.id,nickname:u.nickname,avatar:u.avatar,entry,ready:false,status:'waiting',round:0,bank:entry,lastChest:null,lastOutcome:null,payout:0,joinedAt:now()}}
  async function handle(req,res,url){
    ...
    if(url.pathname==='/api/treasure-raid/rooms'&&req.method==='POST'){
      ...
      try{walletChange(u.id,-entry,'treasure_raid_entry',`${name} 참가금 ${formatMoney(entry)}G`);const p=createPlayer(u,entry),r={id,name,entry,maxPlayers,hostId:u.id,phase:'waiting',players:[p],createdAt:t,updatedAt:t,version:1};rooms.set(id,r);escrowSet(id,u.id,entry,'treasure_raid');pushRefresh();json(res,201,{room:roomPublic(r,u.id),user:userPublic(u.id)});}catch(e){json(res,400,{error:e.message})}return true;
    }
    ...
    const r=rooms.get(m[1]);if(!r){json(res,404,{error:'보물 레이드 방을 찾을 수 없어.'});return true}const op=m[2]||'';
    if(!op&&req.method==='GET'){const p=roomPlayer(r,u.id);if(!p){json(res,403,{error:'이 원정대 참가자가 아니야.'});return true}json(res,200,{room:roomPublic(r,u.id),user:userPublic(u.id)});return true}
    if(op==='join'&&req.method==='POST'){
      if(roomPlayer(r,u.id)){json(res,200,{room:roomPublic(r,u.id),user:userPublic(u.id)});return true}
      if(hasUser(u.id)||isUserBusy?.(u.id)){json(res,409,{error:'이미 다른 게임방에 참가 중이야.'});return true}if(r.phase!=='waiting'){json(res,409,{error:'이미 원정이 시작됐어.'});return true}if(r.players.length>=r.maxPlayers){json(res,409,{error:'원정대가 가득 찼어.'});return true}if(Number(u.balance)<r.entry){json(res,400,{error:'참가금보다 게임머니가 부족해.'});return true}
      try{walletChange(u.id,-r.entry,'treasure_raid_entry',`${r.name} 참가금 ${formatMoney(r.entry)}G`);const p=createPlayer(u,r.entry);r.players.push(p);escrowSet(r.id,u.id,r.entry,'treasure_raid');touch(r);pushRefresh();json(res,200,{room:roomPublic(r,u.id),user:userPublic(u.id)});}catch(e){json(res,400,{error:e.message})}return true;
    }
    const p=roomPlayer(r,u.id);if(!p){json(res,403,{error:'이 원정대 참가자가 아니야.'});return true}
    if(op==='ready'&&req.method==='POST'){
      if(r.phase!=='waiting'){json(res,409,{error:'이미 원정이 시작됐어.'});return true}p.ready=!p.ready;p.status=p.ready?'ready':'waiting';touch(r);pushRefresh();json(res,200,{room:roomPublic(r,u.id),user:userPublic(u.id)});return true;
    }
    if(op==='start'&&req.method==='POST'){
      if(Number(r.hostId)!==Number(u.id)){json(res,403,{error:'방장만 원정을 시작할 수 있어.'});return true}if(r.phase!=='waiting'){json(res,409,{error:'이미 시작된 원정이야.'});return true}if(r.players.length<2){json(res,409,{error:'최소 2명이 필요해.'});return true}if(r.players.some(x=>!x.ready)){json(res,409,{error:'모든 참가자가 준비 완료해야 시작할 수 있어.'});return true}
    ...
    if(op==='continue'&&req.method==='POST'){
      if(r.phase!=='playing'||p.status!=='decision'){json(res,409,{error:'계속 도전할 수 있는 상태가 아니야.'});return true}if(p.round>=MAX_ROUNDS){settlePlayer(r,p,'최종 라운드 정산');json(res,200,{room:roomPublic(r,u.id),user:userPublic(u.id)});return true}p.round+=1;p.status='choosing';p.lastChest=null;p.lastOutcome=null;touch(r);pushRefresh();json(res,200,{room:roomPublic(r,u.id),user:userPublic(u.id)});return true;
    }
    if(op==='cashout'&&req.method==='POST'){
      if(r.phase!=='playing'||!['decision','choosing'].includes(p.status)){json(res,409,{error:'현재는 탈출 정산할 수 없어.'});return true}settlePlayer(r,p,'자진 탈출 정산');json(res,200,{room:roomPublic(r,u.id),user:userPublic(u.id)});return true;
    }
    if(op==='leave'&&req.method==='POST'){
      let payout=0;try{if(r.phase==='waiting')payout=refundWaiting(r,p);else if(r.phase==='playing'&&['choosing','decision'].includes(p.status))payout=settlePlayer(r,p,'원정 중단 정산');else payout=Number(p.payout||0);removePlayer(r,p);pushRefresh();json(res,200,{ok:true,payout,user:userPublic(u.id)});}catch(e){json(res,400,{error:e.message})}return true;
    }
    json(res,405,{error:'지원하지 않는 보물 레이드 요청이야.'});return true;
  }
  return {handle,hasUser,findUserRoom,roomCount:()=>rooms.size};
};