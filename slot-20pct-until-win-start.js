...
// 777 jackpot chance is exactly 20% per spin while this event is active.
...
// The persisted state also prevents a server restart from reactivating the event.
...
const autoRestorePatch = `${sevenJackpotAnchor}\n      if(sevenJackpot&&sevenEventActive){\n        gameStateSet(TEMP_SLOT_777_EVENT_KEY,{active:false,startedAt:gameStateGet(TEMP_SLOT_777_EVENT_KEY,{})?.startedAt||null,endedAt:Date.now(),targetPct:20,winnerUserId:u.id,winnerNickname:u.nickname||null});\n      }`;
