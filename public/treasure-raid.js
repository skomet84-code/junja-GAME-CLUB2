(function(){
  ...
  function readBet(){const v=Math.floor(Number(q('#trEntry')?.value||state.bet||10000000));state.bet=Math.max(1000000,Math.min(100000000,Math.floor(v/1000000)*1000000));if(q('#trEntry'))q('#trEntry').value=state.bet;return state.bet}
  function renderBrowser(){
    ...
    state.root.innerHTML=shell(`<div class="tr-browser-grid"><section class="tr-panel tr-create"><span class="tr-kicker">CREATE RAID ROOM</span><h3>내 배팅금으로 원정 참가</h3><p>같은 방이어도 참가자마다 배팅금이 달라도 돼. 내 보유 G에서 내 금액만 잠금/정산된다.</p><div class="tr-form-row"><input id="trEntry" inputmode="numeric" value="${state.bet}" min="1000000" max="100000000" step="1000000" aria-label="내 배팅금"><select id="trMax"><option value="1">1명 · SOLO</option><option value="2">2명</option><option value="3">3명</option><option value="4" selected>4명</option><option value="5">5명</option><option value="6">6명</option></select></div><div class="tr-quick"><button data-tr-entry="1000000">100만</button><button data-tr-entry="10000000">1,000만</button><button data-tr-entry="100000000">1억</button></div><button id="trCreate" class="tr-primary" style="width:100%">이 금액으로 방 만들기</button><div class="tr-note" style="margin-top:10px">1인 플레이 가능 · 멀티는 각자 금액 선택 · 상자/탈출 결정은 30초 제한 · 시간초과 시 현재 확보 금액으로 자동 탈출</div></section><section class="tr-panel tr-rooms"><div class="tr-headrow"><div><span class="tr-kicker">OPEN EXPEDITIONS</span><h3>열린 원정대</h3></div><button id="trRefresh" class="tr-ghost">새로고침</button></div><div class="tr-room-list">${rooms}</div></section></div><div class="tr-rules"><div class="tr-rule"><b>① 각자 배팅</b><span>100만 · 1,000만 · 1억 기본</span></div><div class="tr-rule"><b>② 30초 선택</b><span>잠수 시 자동 탈출 정산</span></div><div class="tr-rule"><b>③ 독립 진행</b><span>다른 유저가 느려도 내 진행 가능</span></div><div class="tr-rule"><b>④ 더 깊이</b><span>최대 5라운드 · JACKPOT ×100</span></div></div>`);
    bindBrowser();
  }
  ...
  function renderGame(){
    ...
    const r=state.room,p=myPlayer()||{},isHost=Number(r.hostId)===Number(r.selfId),waiting=r.phase==='waiting',playing=r.phase==='playing',allReady=(r.players||[]).every(x=>x.ready);
    ...
    if(waiting){
      ...
      actions+='<button id="trLeave" class="tr-danger">방 나가기 · 전액 환급</button>';
