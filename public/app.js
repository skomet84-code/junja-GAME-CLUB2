...
const MONTH_ICON=['','🌲','🐦','🌸','🌿','🌺','🦋','🍁','🌕','🍂','🦌','🎑','☔'];
function hwatuCard(c,clickable=false,choice=false){if(c.id==='XX')return `<div class="hwatu-card hidden-card"><span>J</span><i>花</i></div>`;const tag=c.ribbon||c.type,flags=`${c.doublePi?' · 쌍피':''}${c.bird?' · 새':''}`,asset=`/art/hwatu/${c.id}.svg`;return `<button class="hwatu-card hwatu-image-card month-${c.m} type-${c.type} ${clickable?'clickable':''} ${choice?'choice-card':''}" ${clickable?`data-hwatu="${c.id}"`:''} ${choice?`data-hwatu-choice="${c.id}"`:''} type="button" aria-label="${c.m}월 ${html(c.name||'화투')} ${html(tag)}${flags}"><img src="${asset}" alt="" draggable="false"><span class="hwatu-card-meta">${html(tag)}${flags}</span></button>`}
