...
function reactionDockHtml(){return `<div class="reaction-dock"><span>QUICK BUBBLE</span>${Object.entries(REACTION_META).map(([k,[e,l]])=>`<button type="button" data-reaction="${k}" title="${html(l)}"><b>${e}</b><small>${html(l)}</small></button>`).join('')}<em>텍스트 입력 없이 정해진 말풍선만 전송</em></div>`}
...
function liveFace(p){return `<div class="live-player-face">${avatarImg(p.avatar,p.nickname,'live-face')}${p.reaction?`<div class="live-speech"><b>${html(p.reaction.emoji)}</b><span>${html(p.reaction.label)}</span></div>`:''}<i class="live-dot"></i></div>`}
function renderLiveFloor(game,members=[]){
  ...
  mount.innerHTML=`<div class="live-floor panel"><div class="live-floor-head"><div><small>LIVE FLOOR · ${html(LIVE_GAME_LABEL[game]||game.toUpperCase())}</small><h3><i></i> 실시간 참여자 <b>${members.length}</b>명</h3></div><span>${['holdem','yut'].includes(game)?'방 참가자는 함께 플레이 · 진행 중엔 같은 게임 유저도 실시간 표시':'같은 게임에 들어온 유저와 동시에 플레이 · 결과는 각자 정산'}</span></div><div class="live-members">${members.map(p=>`<div class="live-member ${Number(p.userId)===Number(me?.id)?'me':''} ${nowMs-Number(p.joinedAt||0)<5000?'just-in':''}">${liveFace(p)}<div><b>${html(p.nickname)}</b><small>${Number(p.userId)===Number(me?.id)?'나 · PLAYING':'ONLINE'}</small></div></div>`).join('')||'<div class="live-empty">아직 이 게임에 접속한 유저가 없어.</div>'}</div><div class="live-reaction-row"><span>말풍선</span>${Object.entries(REACTION_META).map(([k,[e,l]])=>`<button type="button" data-floor-reaction="${k}" title="${html(l)}"><b>${e}</b><small>${html(l)}</small></button>`).join('')}</div></div>`;
...
function startLiveFloor(game){
  ...
  clearInterval(liveHeartbeatTimer);clearInterval(livePollTimer);liveGame=game;liveKnownIds=new Set();liveInitialized=false;
...
function stopLiveFloor(){
  ...
  if(liveGame)leaveLiveFloor(liveGame);liveGame=null;liveKnownIds=new Set();liveInitialized=false;
}
...
function bootMain(){
  ...
  if(!localStorage.getItem('jgc_help_seen_16'))setTimeout(()=>openHelp('lobby'),550);
...
function bindMain(){if(mainBound)return;mainBound=true;
  ...
  $('#bigWheelSpinBtn')?.addEventListener('click',spinBigWheel);$$('[data-wheel-bet]').forEach(b=>b.addEventListener('click',()=>selectBigWheelBet(b.dataset.wheelBet)));$('#sicboRollBtn')?.addEventListener('click',rollSicbo);$$('[data-sicbo-bet]').forEach(b=>b.addEventListener('click',()=>selectSicboBet(b.dataset.sicboBet)));
...
function setHelpTab(tab='lobby'){const ok=['lobby','slot','holdem','yut','seotda','gostop','horse','bigwheel','sicbo'];if(!ok.includes(tab))tab='lobby';$$('[data-help-tab]').forEach(b=>b.classList.toggle('active',b.dataset.helpTab===tab));$$('[data-help-page]').forEach(p=>p.classList.toggle('active',p.dataset.helpPage===tab))}
...
function renderRoom(room){lastRoomVersion=room.version||lastRoomVersion;notifyRoomJoins(room);if(room.game==='holdem')renderHoldem(room);else renderYut(room)}
...
function privacyPanelHtml(){return `<div class="privacy-room-card panel"><div class="privacy-shield">🛡️</div><div><small>PRIVACY MODE</small><h3>텍스트 채팅 없음</h3><p>개인정보 노출 없는 10종 고정 말풍선만 지원합니다.</p></div></div>`}
...
async function spinBigWheel(){if(bigWheelSpinning)return;const input=$('#bigWheelBet'),bet=Math.max(1000,Math.min(100000,Math.floor(Number(input?.value||1000)/1000)*1000));if(input)input.value=bet;bigWheelSpinning=true;const btn=$('#bigWheelSpinBtn');btn.disabled=true;btn.textContent='WHEEL SPINNING...';$('#bigWheelResult').textContent='딜러가 휠을 돌렸습니다…';try{const d=await api('/api/bigwheel/spin',{method:'POST',body:JSON.stringify({bet,key:bigWheelSelected})}),idx=d.result.index,n=BIG_WHEEL_SEGMENTS.length,step=Math.PI*2/n,current=bigWheelAngle%(Math.PI*2),desired=-(idx*step+step/2),turns=7+Math.floor(Math.random()*2),target=current+(turns*Math.PI*2)+(((desired-current)%(Math.PI*2)+Math.PI*2)%(Math.PI*2));const start=performance.now(),from=bigWheelAngle,dur=5200;await new Promise(resolve=>{const frame=t=>{const p=Math.min(1,(t-start)/dur),ease=1-Math.pow(1-p,4);bigWheelAngle=from+(target-from)*ease;drawBigWheel();if(p<1)requestAnimationFrame(frame);else resolve()};requestAnimationFrame(frame)});bigWheelAngle=target;drawBigWheel();const r=d.result;$('#bigWheelResult').innerHTML=r.won?`<b class="wheel-win">🏆 ${html(r.landed.label)} 적중 · ${money(r.payout)}</b>`:`<b>${html(r.landed.label)} 당첨 · 다음 휠에 도전</b>`;me=d.user;updateHeader();if(r.won){fx('win');confetti()}}catch(e){toast(e.message);$('#bigWheelResult').textContent='오류가 발생했어. 다시 시도해줘.'}finally{bigWheelSpinning=false;btn.disabled=false;btn.textContent='SPIN THE WHEEL'}}
...
async function rollSicbo(){if(sicboRolling)return;const input=$('#sicboBet'),bet=Math.max(1000,Math.min(100000,Math.floor(Number(input?.value||1000)/1000)*1000));if(input)input.value=bet;sicboRolling=true;const btn=$('#sicboRollBtn');btn.disabled=true;btn.textContent='ROLLING...';const dice=$$('#sicboDice .die');dice.forEach((d,i)=>{d.classList.add('rolling');d.style.setProperty('--dx',`${(i-1)*18}px`)});let ticker=setInterval(()=>dice.forEach(d=>setDie(d,1+Math.floor(Math.random()*6))),90);try{const d=await api('/api/sicbo/roll',{method:'POST',body:JSON.stringify({bet,key:sicboSelected})});await sleep(1450);clearInterval(ticker);ticker=null;dice.forEach((el,i)=>{el.classList.remove('rolling');setDie(el,d.result.dice[i])});const r=d.result;$('#sicboResult').innerHTML=`<div class="sicbo-result-line"><span>${r.dice.join(' · ')} = <b>${r.total}</b>${r.triple?' · TRIPLE':''}</span><strong class="${r.won?'win':'lose'}">${r.won?'🏆 '+money(r.payout)+' 지급':'MISS'}</strong></div>`;me=d.user;updateHeader();if(r.won){fx('win');if(r.triple||r.bet.mult>=15)confetti()}}catch(e){if(ticker)clearInterval(ticker);dice.forEach(d=>d.classList.remove('rolling'));toast(e.message)}finally{sicboRolling=false;btn.disabled=false;btn.textContent='ROLL DICE'}}
