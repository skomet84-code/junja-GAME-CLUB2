...
function renderGostopMulti(room){
  ...
  root.innerHTML=roomToolbar(room)+`<div class="gostop-board yashimchan-table panel"><div class="matgo-titlebar"><div><small>LIVE MATGO · USER VS USER</small><b>야심찬 맞고 대전</b></div><div class="turn-lamp ${g.turn===side?'mine':''}">${done?'경기 종료':g.pendingChoice?'먹을 패 선택':g.needDecision?'GO / STOP 결정':g.turn===side?'내 차례':'상대 차례'}</div></div><div class="gostop-top"><div>${scoreBox(opp?.nickname||'상대',score(other),g.goCount?.[other]||0,opp?avatarImg(opp.avatar,opp.nickname,'score-face',opp.cosmetics):'')}</div><div class="deck-stack"><div class="hwatu-card hidden-card mini"><span>J</span><i>花</i></div><b>${g.deckCount||0}장</b></div><div>${scoreBox(me.nickname+' · 나',score(side),g.goCount?.[side]||0,avatarImg(me.avatar,me.nickname,'score-face',me.cosmetics))}</div></div><div class="bot-hand-row">${(g.hands?.[other]||[]).map(x=>hwatuCard(x,false)).join('')}</div><div class="floor-title">판 · 같은 월을 맞춰 먹어</div><div class="floor-cards">${(g.floor||[]).map(x=>hwatuCard(x,false,g.pendingChoice?.options?.some(o=>o.id===x.id),(g.hands?.[side]||[]).some(h=>Number(h.m)===Number(x.m)))).join('')}</div><div class="matgo-lower"><div class="matgo-capture-area"><h4>상대 획득패</h4>${capturedSummary(g.captured?.[other]||[])}<h4>내 획득패</h4>${capturedSummary(g.captured?.[side]||[])}</div></div><div class="my-hand-title">내 손패</div><div class="my-hand-row">${(g.hands?.[side]||[]).map(x=>hwatuCard(x,canPlay)).join('')}</div>${g.needDecision&&g.turn===side?'<div class="go-stop-decision"><h3>GO / STOP?</h3><button class="primary" data-mgdecision="go">GO</button><button class="danger" data-mgdecision="stop">STOP</button></div>':''}${done?`<div class="matgo-result ${g.winnerUserId===me.id?'win':g.winnerUserId?'lose':'draw'}"><h2>${g.winnerUserId===me.id?'승리!':g.winnerUserId?'패배':'나가리'}</h2><p>${html(g.result?.reason||'')}</p><b class="matgo-net ${g.winnerUserId===me.id?'plus':g.winnerUserId?'minus':'zero'}">${g.winnerUserId===me.id?`+${money(room.buyIn)} 순이익`:g.winnerUserId?`-${money(room.buyIn)} 손실`:'±0 G · 판돈 환급'}</b><button class="primary gostop-bottom-ready" type="button">${mine?.ready?'✓ READY 완료':'다음 판 READY'}</button></div>`:''}</div>`;
