...
const MONTH_ICON=['','🌲','🐦','🌸','🌿','🌺','🦋','🍁','🌕','🍂','🦌','🎑','☔'];
function hwatuCard(c,clickable=false,choice=false){if(c.id==='XX')return `<div class="hwatu-card hidden-card"><span>J</span><i>花</i></div>`;const tag=c.ribbon||c.type,flags=`${c.doublePi?' · 쌍피':''}${c.bird?' · 새':''}`;return `<button class="hwatu-card hwatu-real month-${c.m} type-${c.type} ${clickable?'clickable':''} ${choice?'choice-card':''}" ${clickable?`data-hwatu="${c.id}"`:''} ${choice?`data-hwatu-choice="${c.id}"`:''} type="button" aria-label="${c.m}월 ${html(tag)}${flags}"><img src="/art/hwatu/${c.id}.svg?v=349" alt="" draggable="false"></button>`}
