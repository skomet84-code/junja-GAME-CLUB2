...
function renderShop(){
  ...
  $('[data-shop-category]',root).forEach(b=>b.onclick=()=>{shopCategory=b.dataset.shopCategory;if(shopCategory!=='character')shopCharacterGender='all',horseMeetTimer=null,horseMeetData=null,horseAnimatedRoundId=null,horseAnimationFrame=null,horseAutoRemaining=0,horseAutoTotal=0,horseAutoBetRound=null,horseAutoBusy=false,slotJackpotPollTimer=null;renderShop()});$$('[data-char-gender]',root).forEach(b=>b.onclick=()=>{shopCharacterGender=b.dataset.charGender;renderShop()});$('#shopOwnedOnly',root)?.addEventListener('change',e=>{shopOwnedOnly=e.target.checked;renderShop()});
...
async function openFriendGiftPicker(target){
  ...
  try{
    ...
    const items=(shopData.items||[]).filter(x=>!x.adminOnly);
    const box=document.createElement('div');box.className='gift-picker-overlay';
    box.innerHTML=`<div class="gift-picker panel"><div class="gift-picker-head"><div><small>JUNJA GIFT</small><h3>${html(target.nickname)}에게 선물</h3></div><button type="button" data-gift-close>✕</button></div><p>내 게임머니로 새 아이템을 구매해서 친구 컬렉션에 바로 선물해.</p><div class="gift-picker-grid">${items.slice().sort((a,b)=>a.price-b.price).map(x=>`<button type="button" data-gift-item="${x.id}" ${(target.ownedItems||[]).some(o=>o.id===x.id)?'disabled':''}><b>${html(x.icon||'◆')}</b><span>${html(x.name)}</span><small>${(target.ownedItems||[]).some(o=>o.id===x.id)?'이미 보유':money(x.price)}</small></button>`).join('')}</div></div>`;
