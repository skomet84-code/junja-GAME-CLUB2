...
const SHOP_CATEGORY_LABELS={all:'전체',frame:'프로필 테두리',costume:'코스튬',title:'칭호',pet:'펫',table_skin:'테이블',card_back:'카드백',bubble_pack:'말풍선'};
...
function renderShop(){
  ...
  const all=shopData.items||[],items=all.filter(x=>(shopCategory==='all'||x.category===shopCategory)&&(!shopOwnedOnly||x.owned));
  const prestige=all.filter(x=>x.rarity==='prestige'),frameCount=all.filter(x=>x.category==='frame').length;
  root.innerHTML=`<div class="luxury-boutique-shell">
    ...
    <div class="shop-stat-strip"><div><b>${all.length}</b><span>전체 아이템</span></div><div><b>${frameCount}</b><span>구매 가능 테두리</span></div><div><b>${prestige.length}</b><span>10억 프레스티지</span></div><div><b>${shopData.ownedCount||0}</b><span>내 컬렉션</span></div></div>
