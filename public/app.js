...
function styledAvatar(avatar=0,alt='플레이어',extra='',cosmetics=null){const idx=Math.abs(Number(avatar||0))%10,frame=cosmeticId(cosmetics,'frame').replace('frame_',''),costume=cosmeticIcon(cosmetics,'costume'),pet=cosmeticIcon(cosmetics,'pet');return `<span class="styled-avatar ${extra} ${frame?'frame-'+frame:''}"><img class="game-face" src="/art/poker-avatars/avatar-${idx}.svg" alt="${html(alt)}">${costume?`<i class="avatar-costume">${html(costume)}</i>`:''}${pet?`<em class="avatar-pet">${html(pet)}</em>`:''}</span>`}
...
const SHOP_CATEGORY_LABELS={all:'전체',costume:'코스튬',frame:'프레임',title:'칭호',pet:'펫',table_skin:'테이블',card_back:'카드백',bubble_pack:'말풍선'};
const SHOP_RARITY_LABELS={common:'COMMON',rare:'RARE',epic:'EPIC',legendary:'LEGEND',mythic:'MYTHIC'};
function shopPreview(item){
  if(item.category==='costume')return `<div class="shop-avatar-preview">${avatarImg(me.avatar,me.nickname,'shop-face',me.cosmetics)}<i>${html(item.icon)}</i></div>`;
  if(item.category==='frame')return `<div class="shop-frame-preview frame-${item.id.replace('frame_','')}">${avatarImg(me.avatar,me.nickname,'shop-face')}</div>`;
  if(item.category==='title')return `<div class="shop-title-preview"><b>${html(me.nickname)}</b><span>${html(item.name)}</span></div>`;
  if(item.category==='pet')return `<div class="shop-pet-preview"><span>${html(item.icon)}</span><b>${html(me.nickname)}</b></div>`;
  if(item.category==='table_skin')return `<div class="shop-table-preview ${item.id}"><i></i><b>JUNJA</b><span>♠ ♥ ♦ ♣</span></div>`;
  if(item.category==='card_back')return `<div class="shop-card-preview ${item.id}"><b>${html(item.icon)}</b><span>JUNJA</span></div>`;
  return `<div class="shop-bubble-preview"><span>${html(item.icon)}</span><b>${html(item.name)}</b></div>`;
}
function renderShop(){
  const root=$('#shopContent');if(!root||!shopData)return;const c=shopData.loadout?.collection||{name:'NEW MEMBER',level:0};
  const items=(shopData.items||[]).filter(x=>(shopCategory==='all'||x.category===shopCategory)&&(!shopOwnedOnly||x.owned));
  root.innerHTML=`<div class="boutique-hero panel"><div class="boutique-me">${avatarImg(me.avatar,me.nickname,'boutique-face',me.cosmetics)}<div><small>MY STYLE</small><h3>${html(me.nickname)}</h3>${equippedTitle(me.cosmetics)?`<span>${html(equippedTitle(me.cosmetics))}</span>`:'<span>칭호 미장착</span>'}<p>${c.icon||'◇'} ${html(c.name)} · ${shopData.ownedCount||0}개 보유</p></div></div><div class="boutique-wallet"><small>AVAILABLE</small><b>${money(me.balance)}</b><em>게임머니로만 구매 · 현금 가치 없음</em></div></div>
  <div class="shop-toolbar panel"><div class="shop-tabs">${Object.entries(SHOP_CATEGORY_LABELS).map(([k,v])=>`<button type="button" data-shop-category="${k}" class="${shopCategory===k?'active':''}">${v}</button>`).join('')}</div><label class="owned-toggle"><input id="shopOwnedOnly" type="checkbox" ${shopOwnedOnly?'checked':''}><span>내가 보유한 아이템만</span></label></div>
  <div class="shop-grid">${items.map(x=>`<article class="shop-item panel rarity-${x.rarity} ${x.equipped?'equipped':''}"><div class="shop-item-top"><span class="rarity">${SHOP_RARITY_LABELS[x.rarity]||x.rarity}</span>${x.equipped?'<b class="equipped-badge">EQUIPPED</b>':x.owned?'<b class="owned-badge">OWNED</b>':''}</div>${shopPreview(x)}<div class="shop-item-copy"><small>${html(SHOP_CATEGORY_LABELS[x.category]||x.category)}</small><h3>${html(x.name)}</h3><p>${html(x.desc)}</p></div><div class="shop-item-foot"><b>${money(x.price)}</b>${x.equipped?`<button class="ghost" data-shop-unequip="${x.category}" type="button">해제</button>`:x.owned?`<button class="primary" data-shop-equip="${x.id}" data-shop-slot="${x.category}" type="button">장착</button>`:`<button class="secondary" data-shop-buy="${x.id}" type="button">구매</button>`}</div></article>`).join('')||'<div class="empty panel">조건에 맞는 아이템이 없어.</div>'}</div>`;
  $$('[data-shop-category]',root).forEach(b=>b.onclick=()=>{shopCategory=b.dataset.shopCategory;renderShop()});$('#shopOwnedOnly',root)?.addEventListener('change',e=>{shopOwnedOnly=e.target.checked;renderShop()});
