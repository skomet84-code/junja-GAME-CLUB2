...
function renderShop(){
  ...
  const all=shopData.items||[],limited=all.filter(x=>x.collection==='limited'),items=all.filter(x=>(shopCategory==='all'||x.category===shopCategory)&&(shopCategory!=='character'||shopCharacterGender==='all'||x.gender===shopCharacterGender)&&(!shopOwnedOnly||x.owned));
