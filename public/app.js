...
const rankingMoney=n=>{
  try{
    let s=String(n??'0').trim().replace(/,/g,'');
    const neg=s.startsWith('-');if(neg)s=s.slice(1);
    if(!/^\d+$/.test(s))return money(n);
    let v=BigInt(s);const original=v;
    if(v<100000000n)return money(n);
    const units=[['경',10000000000000000n],['조',1000000000000n],['억',100000000n]];
    const parts=[];
    for(const [label,size] of units){const q=v/size;if(q>0n){parts.push(String(q)+label);v%=size;}}
    const approx=v>0n?'약 ':'';
    return (neg?'-':'')+approx+parts.join(' ')+' G';
...
function renderShop(){
  ...
  root.innerHTML=`<div class="luxury-boutique-shell">
    ...
    <section class="ultimate-vault panel"><div><small>THE FINAL COLLECTION</small><h3>궁극의 경지</h3><p>태양의 왕관 · 심연의 궤도 · 천상의 날개</p><span>프로필 테두리 3종 · 각 10조 G · 구매 즉시 자동 장착</span></div><button type="button" data-shop-category="ultimate">✦ 궁극 컬렉션 보기 →</button></section>
...
async function startSoloHoldem(){if(!confirm(`홀덤 AI 테이블에 ${money(Math.min(10000000000000,Number(me.balance)))}를 가져갈까? 입장 한도는 10조이며 초과 금액은 지갑에 남아. 블라인드는 입장 칩에 따라 최대 5억 / 10억이야. 나갈 때 남은 칩은 다시 정산돼.`))return;try{const d=await api('/api/solo/holdem/start',{method:'POST',body:'{}'});me=d.user;updateHeader();renderSoloHoldem(d.room)}catch(e){toast(e.message)}}
