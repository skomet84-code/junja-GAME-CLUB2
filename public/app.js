...
async function ensureAudioUnlocked(){if(audioUnlocking)return audioUnlocked;audioUnlocking=true;try{const c=audioContext();if(c&&c.state!=='running')await c.resume();audioUnlocked=!!c&&c.state==='running'}catch{audioUnlocked=false}audioUnlocking=false;return audioUnlocked}
...
async function pickDailyDraw(number){const status=$('#dailyDrawStatus');if(status)status.textContent=`${number}번 결과 확인 중...`;try{const d=await api('/api/daily-draw/pick',{method:'POST',body:JSON.stringify({number})});me=d.user;updateHeader();renderDailyDraw(d.state);fx('win');}catch(e){toast(e.message);try{renderDailyDraw(await api('/api/daily-draw'))}catch{}}}
...
const SHOP_CATEGORY_LABELS={all:'전체',ultimate:'✦ 궁극 · 10조',character:'캐릭터',frame:'프로필 테두리',costume:'코스튬',title:'칭호',pet:'펫',table_skin:'테이블',card_back:'카드백',bubble_pack:'말풍선'};
const SHOP_RARITY_LABELS={common:'COMMON',rare:'RARE',epic:'EPIC',legendary:'LEGEND',mythic:'MYTHIC',prestige:'PRESTIGE 1B',limited:'LIMITED',ultimate:'ULTIMATE · 궁극'};
function shopPreview(item){
  ...
  if(item.category==='frame')return `<div class="shop-frame-preview preview-${item.rarity}">${avatarImg(me.avatar,me.nickname,'shop-face',current)}<span class="frame-preview-label">${item.collection==='ultimate'?'ULTIMATE COLLECTION':'PROFILE BORDER'}</span></div>`;
...
function renderShop(){
  ...
  const all=shopData.items||[],limited=all.filter(x=>x.collection==='limited'),items=all.filter(x=>(shopCategory==='all'||x.category===shopCategory||shopCategory==='ultimate'&&x.collection==='ultimate')&&(shopCategory!=='character'||shopCharacterGender==='all'||x.gender===shopCharacterGender)&&(!shopOwnedOnly||x.owned));
  ...
  root.innerHTML=`<div class="luxury-boutique-shell">
    ...
    <section class="ultimate-vault panel"><div><small>THE FINAL COLLECTION</small><h3>궁극의 경지</h3><p>태양의 왕관 · 심연의 궤도 · 천상의 날개</p><span>프로필 테두리 3종 · 각 10조 G · 장착 시 전용 효과</span></div><button type="button" data-shop-category="ultimate">✦ 궁극 컬렉션 보기 →</button></section><div class="prestige-showcase panel"><div><small>PRESTIGE VAULT</small><h3>10억 G 레전드 컬렉션</h3><p>쉽게 살 수 없는 아이템이 있어야 진짜 목표가 생겨. 소유하면 프로필과 게임 테이블에서 바로 티가 나.</p></div><div class="prestige-mini-grid">${prestige.slice(0,4).map(x=>`<button type="button" data-prestige-jump="${x.category}"><b>${html(x.icon)}</b><span>${html(x.name)}</span><em>1,000,000,000 G</em></button>`).join('')}</div></div>
    ${limited.length?`<section class="limited-collection panel"><div class="limited-collection-head"><small>ONLY FOR A LIMITED TIME</small><h3>👑 LIMITED COLLECTION</h3><p>여성 5종 · 각 10,000,000,000 G</p></div><div class="limited-collection-grid">${limited.map(x=>`<button type="button" data-limited-jump="${x.gender}"><img src="${html(x.asset)}" alt="${html(x.name)}"><b>${html(x.name)}</b><span>100억 G</span></button>`).join('')}</div></section>`:''}
    ...
    <div class="shop-grid">${items.map(x=>`<article class="shop-item panel rarity-${x.rarity} ${x.equipped?'equipped':''} ${x.featured?'featured-item':''}"><div class="shop-item-top"><span class="rarity">${SHOP_RARITY_LABELS[x.rarity]||x.rarity}</span>${x.collection==='ultimate'?'<b class="prestige-badge">10조 G</b>':x.featured?'<b class="prestige-badge">1 BILLION</b>':x.equipped?'<b class="equipped-badge">EQUIPPED</b>':x.owned?'<b class="owned-badge">OWNED</b>':''}</div>${shopPreview(x)}<div class="shop-item-copy"><small>${html(SHOP_CATEGORY_LABELS[x.category]||x.category)}</small>${x.hideName?'':`<h3>${html(x.name)}</h3>`}<p>${html(x.desc)}</p>${x.perk?`<div class="shop-perk">⚡ ${x.perk.slotLuckPct?`희귀 심볼 가중치 +${x.perk.slotLuckPct}%`:''}${x.perk.slotLuckPct&&x.perk.dailyBonusPct?' · ':''}${x.perk.dailyBonusPct?`출석 보너스 +${x.perk.dailyBonusPct}%`:''}</div>`:''}</div><div class="shop-item-foot"><div class="shop-price"><small>PRICE</small><b>${money(x.price)}</b></div>${x.equipped?`<button class="ghost" data-shop-unequip="${x.category}" type="button">장착 해제</button>`:x.owned?`<button class="primary" data-shop-equip="${x.id}" data-shop-slot="${x.category}" type="button">장착하기</button>`:x.rankTier&&!x.rankShopUnlocked?`<button class="secondary buy-luxury" type="button" disabled>🔒 신분 TIER ${x.rankTier}</button>`:`<button class="secondary buy-luxury" data-shop-buy="${x.id}" type="button">${x.rankTier?`👑 TIER ${x.rankTier} · `:""}구매하기</button>`}</div></article>`).join('')||'<div class="empty panel">조건에 맞는 아이템이 없어.</div>'}</div>
  </div>`;
...
function bindFriendTransfer(){
  ...
  send.onclick=async()=>{
    ...
    const preview=transferPreview();if(preview.total>Number(me?.balance||0)){toast(`수수료 포함 ${money(preview.total)}가 필요해.`);return}const ok=confirm(`${friendTransferTarget.nickname}님에게 ${money(n)}를 보낼까?\n신분 수수료 ${money(preview.fee)} (${preview.pct}%) · 총 ${money(preview.total)} 차감\n전송 후에는 자동 취소되지 않아.`);if(!ok)return;
    friendTransferBusy=true;send.disabled=true;send.textContent='보내는 중...';try{const d=await api('/api/wallet/transfer',{method:'POST',body:JSON.stringify({targetId:friendTransferTarget.id,amount:n})});me=d.user;updateHeader();const profileBalance=$('#profileBalance');if(profileBalance)profileBalance.textContent=window.JunjaMoney.exact(me.balance)+' G';toast(`🎁 ${friendTransferTarget.nickname}님에게 ${money(n)} 보냈어!`);amount.value=Math.min(10000,Number(me.balance||0));resetFriendTarget('전송 완료. 또 보내려면 친구를 다시 확인해줘.');nick.value='';fx('win')}catch(e){toast(e.message);send.disabled=false}finally{friendTransferBusy=false;send.textContent='선택한 친구에게 보내기'}
  };
}
...
function bindAdminRoomRows(){
  document.querySelectorAll('[data-admin-close-room]').forEach(b=>b.onclick=async()=>{if(!confirm('이 방을 강제 종료하고 참가자 판돈을 복구할까?'))return;try{await api(`/api/admin/rooms/${b.dataset.adminCloseRoom}/close`,{method:'POST',body:'{}'});toast('게임방 종료 완료');await loadAdmin($('#adminSearch')?.value.trim()||'')}catch(e){toast(e.message)}});
...
function slotBetUnlimited(){return Number(me?.rank?.level||0)>=9}
...
function pokerTableHtml(room,h,solo=false,deal=false){
  ...
  for(let i=0;i<room.maxPlayers;i++){
    ...
    seats+=`<div class="seat s${i} ${folded?'folded-seat':''}"><div class="hole">${hole}</div><div class="player-box ${h?.turnUserId===p.userId?'turn':''} ${folded?'folded':''}">${reactionBubble(room,p.userId)}${folded?'<div class="folded-overlay" aria-label="다이"><b>다이</b><small>FOLDED</small></div>':''}${pokerFaceHtml(p)}<div class="player-meta"><div class="player-name">${html(p.nickname)} ${h?.dealerSeat===p.seat?'<span class="dealer-dot">D</span>':''}</div><div class="player-stack" title="${window.JunjaMoney.exact(p.stack)} G">${money(p.stack)}</div>${hp?`<div class="player-bet">BET ${money(hp.roundBet)}${flags}</div>`:''}</div></div></div>`
  }
...
function updateHorseStartState(){const r=horseMeetData,type=$('#horseBetType')?.value||'win',ids=horseSelectedIds(),need=type==='win'?1:2,btn=$('#horseStartBtn');if(!btn)return;const betting=r?.phase==='betting',already=!!r?.myBet,canBet=betting&&!already&&ids.length===need&&!horseAutoBusy;btn.disabled=!betting||horseAutoBusy||(already&&r.myReady)||(!already&&!canBet);if(already)btn.textContent=r.myReady?'✓ 준비 완료 · 출발 대기':'🏁 준비하고 출발';else if(!betting)btn.textContent=r?.phase==='running'?'🏇 경주 중':'결과 확인 중';else btn.textContent=canBet?`💰 베팅 접수 · x${horseOddsFor(type,ids)}`:(need===1?'🏇 말 1마리를 선택해줘':'🏇 말 2마리를 차례로 선택해줘')}
...
function renderHorseParticipants(r){const root=$('#horseLiveBettors');if(!root)return;const ps=r.participants||[];root.innerHTML=ps.length?`<b>참가자 ${ps.length}</b>${ps.map(p=>`<span>${avatarImg(p.avatar,p.nickname,'mini-live-face',p.cosmetics)}<em>${html(p.nickname)}</em><small>${p.ready?'✅ 준비':'⏳ 대기'} · ${p.type==='win'?'단승':p.type==='quinella'?'복승':'쌍승'} · ${p.picks.join('/')}</small></span>`).join('')}`:'<span>베팅 후 준비하면 경주가 시작돼.</span>'}
function renderHorseMeet(r){if(!r?.card)return;const changed=!horseMeetData||horseMeetData.id!==r.id;horseMeetData=r;horseCardData=r.card;if(changed||!$('#horseTrack .race-lane'))horseTrackBuild(r.card);else if($('#horseTrack').querySelectorAll('.race-lane').length!==7)horseTrackBuild(r.card);renderHorsePickGrid();renderHorseOdds();renderHorseOddsBoard();renderHorseParticipants(r);const clock=$('#horseMeetClock');if(clock)clock.innerHTML=`<small>${r.phase==='betting'?'READY':r.phase==='running'?'RACE':'RESULT'}</small><b>${r.phase==='betting'?`${(r.participants||[]).filter(p=>p.ready).length}/${(r.participants||[]).length} 준비`:r.phase==='running'?'RUNNING':'정산 중'}</b>`;const ov=$('#raceOverlay');if(ov){ov.classList.toggle('hidden',r.phase==='running');ov.innerHTML=r.phase==='betting'?'<b>BET & READY</b><span>베팅한 참가자가 준비하면 출발합니다.</span>':r.phase==='result'?'<b>OFFICIAL RESULT</b><span>잠시 후 다음 경주가 열립니다.</span>':'<b>RACE</b>'}if(r.phase==='running'&&r.order)animateHorseMeet(r);else if(r.phase==='result'&&r.order){setHorseFinalPositions(r);horseLastResult={round:r};renderHorseMeetResult(r)}else if(r.phase==='betting'){resetHorsePositions();if(r.myBet)renderHorseBetReceipt(r);else if(horseLastResult?.round)renderHorseMeetResult(horseLastResult.round);else $('#horseResult').innerHTML='<div class="race-result-empty"><b>BET & READY</b><p>말과 금액을 선택해 베팅한 뒤 준비를 눌러 출발해.</p></div>'}updateHorseStartState();updateHorseAutoUi()}
...
async function startHorseRace(auto=false){const r=horseMeetData;if(!r||r.phase!=='betting'||horseAutoBusy)return false;if(r.myBet){if(r.myReady)return false;horseAutoBusy=true;updateHorseStartState();try{const d=await api('/api/horse/ready',{method:'POST',body:'{}'});horseMeetData=d.round;if(d.user){me=d.user;updateHeader()}renderHorseMeet(d.round);return true}catch(e){if(!auto)toast(e.message);return false}finally{horseAutoBusy=false;updateHorseStartState()}}const type=$('#horseBetType').value,ids=horseSelectedIds(),need=type==='win'?1:2;if(ids.length!==need){if(!auto)toast(need===1?'말 한 마리를 선택해줘.':'서로 다른 말 두 마리를 선택해줘.');return false}const bet=Math.max(1000,Math.min(10000000,Math.floor(Number($('#horseBet').value||1000)/1000)*1000));$('#horseBet').value=bet;horseAutoBusy=true;updateHorseStartState();try{const d=await api('/api/horse/bet',{method:'POST',body:JSON.stringify({type,picks:ids,bet})});horseMeetData=d.round;horseCardData=d.round.card;if(d.user){me=d.user;updateHeader()}renderHorseMeet(d.round);return true}catch(e){if(!auto)toast(e.message);return false}finally{horseAutoBusy=false;updateHorseStartState()}}
...
function rankFreeInfo(kind){const p=me?.rank?.perks||{},limit=Number(p[kind==='slot'?'freeSlots':'freeBigWheel']||0),bet=Number(p.freeSlotBet||0);return {limit,bet}}
function updateRankFreeUi(){const sf=me?.rank?.freePlay?.slot||{used:0,limit:rankFreeInfo('slot').limit,left:rankFreeInfo('slot').limit},wf=me?.rank?.freePlay?.wheel||{used:0,limit:rankFreeInfo('wheel').limit,left:rankFreeInfo('wheel').limit},ss=$('#slotRankFreeStatus'),ws=$('#wheelRankFreeStatus'),sb=$('#slotRankFreeBtn'),wb=$('#wheelRankFreeBtn');if(ss){const bet=rankFreeInfo('slot').bet;ss.textContent=sf.limit?`오늘 ${sf.used}/${sf.limit} 사용 · ${sf.left}회 남음 · ${money(bet)} 고정`:'현재 신분 무료권 없음';}if(ws){const bet=rankFreeInfo('wheel').bet;ws.textContent=wf.limit?`오늘 ${wf.used}/${wf.limit} 사용 · ${wf.left}회 남음 · ${money(bet)} 고정`:'현재 신분 무료권 없음';}if(sb){sb.disabled=!sf.left;sb.classList.toggle('active',rankFreeSlotNext)}if(wb){wb.disabled=!wf.left;wb.classList.toggle('active',rankFreeWheelNext)}}
...
// GOSTOP MULTIPLAYER
function renderGostopMulti(room){
  currentRoomId=room.id;currentGame='gostop';
  $('#gostopBrowser')?.classList.add('hidden');const root=$('#gostopRoom');root?.classList.remove('hidden');if(!root)return;
  const g=room.gostop,mine=room.players.find(p=>p.userId===me.id),opp=room.players.find(p=>p.userId!==me.id);
  if(!g){root.innerHTML=roomToolbar(room)+'<div class="panel"><h3>맞고 대전 대기실</h3><p>두 명 모두 READY 후 방장이 게임 시작을 눌러줘.</p></div>';bindRoomCommon(root,room);return}
  const side=String(g.userA)===String(me.id)?'a':'b',other=side==='a'?'b':'a',done=g.phase==='complete',canPlay=!done&&!g.needDecision&&!g.pendingChoice&&g.turn===side;
  const score=(x)=>g.score?.[x]||{score:0,g:0,a:0,r:0,p:0,combos:[]};
  const monthGroups={};for(const card of (g.hands?.[side]||[])){if(card.id==='XX'||card.bonusPi)continue;(monthGroups[card.m]??=[]).push(card)}const specials=canPlay?Object.entries(monthGroups).filter(([,cards])=>cards.length>=3).map(([m])=>{const floorCount=(g.floor||[]).filter(x=>Number(x.m)===Number(m)).length;return floorCount===1?`<button class="danger" data-mgspecial="bomb" data-month="${m}" type="button">💣 ${m}월 폭탄</button>`:`<button class="primary" data-mgspecial="shake" data-month="${m}" type="button">🔥 ${m}월 흔들기</button>`}).join(''):'';
  root.innerHTML=roomToolbar(room)+`<div class="gostop-board yashimchan-table panel"><div class="matgo-titlebar"><div><small>LIVE MATGO · USER VS USER</small><b>야심찬 맞고 대전</b></div><div class="turn-lamp ${g.turn===side?'mine':''}">${done?'경기 종료':g.pendingChoice?'먹을 패 선택':g.needDecision?'GO / STOP 결정':g.turn===side?'내 차례':'상대 차례'}</div></div><div class="gostop-top"><div>${scoreBox(opp?.nickname||'상대',score(other),g.goCount?.[other]||0,opp?avatarImg(opp.avatar,opp.nickname,'score-face',opp.cosmetics):'')}</div><div class="deck-stack"><div class="hwatu-card hidden-card mini"><span>J</span><i>花</i></div><b>${g.deckCount||0}장</b></div><div>${scoreBox(me.nickname+' · 나',score(side),g.goCount?.[side]||0,avatarImg(me.avatar,me.nickname,'score-face',me.cosmetics))}</div></div><div class="bot-hand-row">${(g.hands?.[other]||[]).map(x=>hwatuCard(x,false)).join('')}</div><div class="floor-title">판 · 같은 월을 맞춰 먹어</div><div class="floor-cards">${(g.floor||[]).map(x=>hwatuCard(x,false,g.pendingChoice?.options?.some(o=>o.id===x.id),(g.hands?.[side]||[]).some(h=>Number(h.m)===Number(x.m)))).join('')}</div><div class="matgo-lower"><div class="matgo-capture-area"><h4>상대 획득패</h4>${capturedSummary(g.captured?.[other]||[])}<h4>내 획득패</h4>${capturedSummary(g.captured?.[side]||[])}</div></div><div class="my-hand-title">내 손패</div><div class="my-hand-row">${(g.hands?.[side]||[]).map(x=>hwatuCard(x,canPlay)).join('')}</div>${specials?'<div class="go-stop-decision"><h3>특수패</h3>'+specials+'</div>':''}${g.needDecision&&g.turn===side?'<div class="go-stop-decision"><h3>GO / STOP?</h3><button class="primary" data-mgdecision="go">GO</button><button class="danger" data-mgdecision="stop">STOP</button></div>':''}${done?`<div class="matgo-result ${g.winnerUserId===me.id?'win':g.winnerUserId?'lose':'draw'}"><h2>${g.winnerUserId===me.id?'승리!':g.winnerUserId?'패배':'나가리'}</h2><p>${html(g.result?.reason||'')}</p><b class="matgo-net ${g.winnerUserId===me.id?'plus':g.winnerUserId?'minus':'zero'}">${g.winnerUserId===me.id?`+${money(g.result?.payout||room.buyIn)} 순이익 · ${g.result?.base||0}점 ×${g.result?.multiplier||1}`:g.winnerUserId?`-${money(g.result?.payout||room.buyIn)} 손실 · ${g.result?.base||0}점 ×${g.result?.multiplier||1}`:'±0 G · 판돈 환급'}</b><button class="primary gostop-bottom-ready" type="button">${mine?.ready?'✓ READY 완료':'다음 판 READY'}</button></div>`:''}</div>`;
  ...
  document.querySelectorAll('[data-hwatu-choice]').forEach(b=>b.onclick=async()=>{try{const d=await api(`/api/rooms/${room.id}/gostop/play`,{method:'POST',body:JSON.stringify({cardId:g.pendingChoice?.card?.id||'',choiceId:b.dataset.hwatuChoice})});renderRoom(d.room)}catch(e){toast(e.message)}});
  $$('[data-mgspecial]',root).forEach(b=>b.onclick=async()=>{try{b.disabled=true;const d=await api(`/api/rooms/${room.id}/gostop/play`,{method:'POST',body:JSON.stringify({action:b.dataset.mgspecial,month:Number(b.dataset.month)})});renderRoom(d.room)}catch(e){toast(e.message)}finally{b.disabled=false}});
...
const MONTH_ICON=['','🌲','🐦','🌸','🌿','🌺','🦋','🍁','🌕','🍂','🦌','🎑','☔'];
function hwatuTypeLabel(c){if(c?.doublePi)return '쌍피';if(c?.type==='광')return '광';if(c?.type==='열끗')return '열끗';if(c?.type==='띠')return c?.ribbon||'띠';return '피'}
function hwatuCard(c,clickable=false,choice=false,monthMatch=false){if(c?.bonusPi)return `<div class="hwatu-card hwatu-real type-피 bonus-pi"><span>${c.doublePi?'BONUS ×2':'BONUS'}</span><small class="hwatu-kind">${c.doublePi?'쌍피':'피'}</small></div>`;if(c.id==='XX')return `<div class="hwatu-card hidden-card"><span>J</span><i>花</i></div>`;const tag=c.ribbon||c.type,flags=`${c.doublePi?' · 쌍피':''}${c.bird?' · 새':''}`;return `<button class="hwatu-card hwatu-real month-${c.m} type-${c.type} ${clickable?'clickable':''} ${choice?'choice-card':''} ${monthMatch?'hand-match':''}" ${clickable?`data-hwatu="${c.id}"`:''} ${choice?`data-hwatu-choice="${c.id}"`:''} type="button" aria-label="${c.m}월 ${html(tag)}${flags}"><img src="/art/hwatu/${c.id}.svg?v=349" alt="" draggable="false"><small class="hwatu-kind">${html(hwatuTypeLabel(c))}</small></button>`}
...
function scoreBox(label,s,go,face=''){const total=Number(s.score||0)+Number(go||0);return `<div class="gscore ${total>=1?'score-active':''}"><div class="gscore-who">${face}<span>${html(label)}</span></div><b>${total}점</b><small>광 ${s.g} · 열 ${s.a} · 띠 ${s.r} · 피 ${s.p} · ${go}고</small>${s.combos?.length?`<div class="matgo-combos">${s.combos.map(x=>`<em>${html(x)}</em>`).join('')}</div>`:''}</div>`}
...
function renderGostop(g){$('#gostopStart').classList.add('hidden');const root=$('#gostopTable');root.classList.remove('hidden');const done=g.phase==='complete',canPlay=!done&&!g.needDecision&&!g.pendingChoice&&g.turn==='user';const monthGroups={};for(const card of (g.hands?.user||[])){if(card.id==='XX'||card.bonusPi)continue;(monthGroups[card.m]??=[]).push(card)}const soloSpecials=canPlay?Object.entries(monthGroups).filter(([,cards])=>cards.length>=3).map(([m])=>{const floorCount=(g.floor||[]).filter(x=>Number(x.m)===Number(m)).length;return floorCount===1?`<button class="danger" data-gspecial="bomb" data-month="${m}" type="button">💣 ${m}월 폭탄</button>`:`<button class="primary" data-gspecial="shake" data-month="${m}" type="button">🔥 ${m}월 흔들기</button>`}).join(''):'';const result=g.result,events=(g.events||[]).map(e=>`<li class="${e.tone||''}">${html(e.text)}</li>`).join('');root.innerHTML=`<div class="gostop-board yashimchan-table panel"><div class="matgo-titlebar"><div><small>YASHIMCHAN ORIGINAL</small><b>야심찬 맞고</b></div>${done?'<button class="turn-lamp mine end-gostop" type="button">경기 종료</button>':`<div class="matgo-live-actions"><div class="turn-lamp ${g.turn==='user'?'mine':''}">${g.pendingChoice?'먹을 패 선택':g.needDecision?'GO / STOP 결정':g.turn==='user'?'내 차례':'J-BOT 생각 중'}</div><button class="turn-lamp end-gostop" type="button">나가기</button></div>`}</div><div class="gostop-top"><div>${scoreBox('J-BOT',g.score.bot,g.goCount.bot,botFace('gostop','score-face'))}</div><div class="deck-stack"><div class="hwatu-card hidden-card mini"><span>J</span><i>花</i></div><b>${g.deckCount}장</b>${g.lastDraw?`<small>뒤집은 패 ${g.lastDraw.m}월</small>`:''}</div><div>${scoreBox(`${me.nickname} · 나`,g.score.user,g.goCount.user,avatarImg(me.avatar,me.nickname,'score-face',me.cosmetics))}</div></div><div class="bot-hand-row">${g.hands.bot.map(c=>hwatuCard(c,false)).join('')}</div><div class="floor-title">판 · 같은 월을 맞춰 먹어</div><div class="floor-cards">${g.floor.map(c=>hwatuCard(c,false,g.pendingChoice?.options?.some(x=>x.id===c.id),g.hands.user.some(h=>Number(h.m)===Number(c.m)))).join('')||'<span class="empty-floor">싹쓸이!</span>'}</div><div class="matgo-lower"><div class="matgo-capture-area"><h4>J-BOT 획득패</h4>${capturedSummary(g.captured.bot)}<h4>내 획득패</h4>${capturedSummary(g.captured.user)}</div><ul class="matgo-event-log">${events||'<li>패를 선택해 시작해.</li>'}</ul></div><div class="my-hand-title">내 손패 ${canPlay?'· 낼 패를 눌러':'· 잠시만'}</div><div class="my-hand-row">${g.hands.user.map(c=>hwatuCard(c,canPlay)).join('')}</div>${soloSpecials?`<div class="go-stop-decision"><h3>특수패</h3>${soloSpecials}</div>`:''}${g.pendingChoice?`<div class="matgo-choice"><small>같은 월 패가 2장</small><h3>어느 패를 먹을까?</h3><p>빛나는 바닥패 중 하나를 선택해.</p></div>`:''}${g.needDecision?`<div class="go-stop-decision"><div><small>SCORE UP · ${g.goCount.user}고</small><h3>${g.score.user.score+g.goCount.user}점! 승부를 계속할까?</h3><p>3고부터 최종 금액이 2배씩 커져.</p></div><button class="primary" data-gdecision="go" type="button">GO</button><button class="danger" data-gdecision="stop" type="button">STOP</button></div>`:''}${done?`<div class="matgo-result ${g.winner==='user'?'win':'lose'}"><small>${g.winner==='user'?'VICTORY':g.winner==='bot'?'DEFEAT':'DRAW'}</small><h2>${g.winner==='user'?'야심찬 승리!':g.winner==='bot'?'J-BOT 승리':'나가리'}</h2><p>${html(result?.reason||'')} · ${result?.base||0}점 × ${result?.multiplier||1}배</p>${result?.bonuses?.length?`<div>${result.bonuses.map(x=>`<span>${html(x)}</span>`).join('')}</div>`:''}<b class="matgo-net ${g.winner==='user'?'plus':g.winner==='bot'?'minus':'zero'}">${g.winner==='user'?`+${money(Math.max(0,Number(result?.payout||0)-Number(g.bet||0)))} 순이익`:g.winner==='bot'?`-${money(g.bet||0)} 손실`:'±0 G'}</b><button class="primary new-gostop" type="button">같은 자리에서 다음 판</button></div>`:''}</div>`;$$('[data-hwatu]',root).forEach(b=>b.onclick=async()=>{try{b.disabled=true;const d=await api('/api/solo/gostop/play',{method:'POST',body:JSON.stringify({cardId:b.dataset.hwatu})});if(d.user){me=d.user;updateHeader()}renderGostop(d.game)}catch(e){toast(e.message)}});$$('[data-hwatu-choice]',root).forEach(b=>b.onclick=async()=>{try{b.disabled=true;const d=await api('/api/solo/gostop/play',{method:'POST',body:JSON.stringify({cardId:g.pendingChoice?.card?.id||'',choiceId:b.dataset.hwatuChoice})});if(d.user){me=d.user;updateHeader()}renderGostop(d.game)}catch(e){toast(e.message)}});$$('[data-gspecial]',root).forEach(b=>b.onclick=async()=>{try{b.disabled=true;const d=await api('/api/solo/gostop/play',{method:'POST',body:JSON.stringify({action:b.dataset.gspecial,month:Number(b.dataset.month)})});if(d.user){me=d.user;updateHeader()}renderGostop(d.game)}catch(e){toast(e.message)}finally{b.disabled=false}});$$('[data-gdecision]',root).forEach(b=>b.onclick=async()=>{try{const d=await api('/api/solo/gostop/decision',{method:'POST',body:JSON.stringify({decision:b.dataset.gdecision})});if(d.user){me=d.user;updateHeader()}renderGostop(d.game)}catch(e){toast(e.message)}});const exitGostop=async()=>{try{const d=await api('/api/solo/gostop/reset',{method:'POST',body:'{}'});if(d.user)me=d.user;else await refreshMe();updateHeader();root.classList.add('hidden');$('#gostopStart').classList.remove('hidden')}catch(e){toast(e.message)}};const nextGostop=async()=>{try{const bet=Math.max(5000,Math.min(1000000,Number(g.bet||5000)));const d=await api('/api/solo/gostop/start',{method:'POST',body:JSON.stringify({bet})});if(d.user){me=d.user;updateHeader()}renderGostop(d.game);requestAnimationFrame(()=>root.scrollIntoView({behavior:'smooth',block:'start'}))}catch(e){toast(e.message)}};$('.new-gostop',root)?.addEventListener('click',nextGostop);$('.end-gostop',root)?.addEventListener('click',exitGostop)}
...
function sevenActions(g){
  ...
  const l=g.legal||{},current=Number(g.currentBet||0),myRound=Number(g.roundBet?.user||0),fallbackMax=Math.min(myRound+Number(g.stack?.user||0),Number(g.roundBet?.bot||0)+Number(g.stack?.bot||0));
  const call=Math.max(0,Number(l.toCall??(current-myRound))),serverMax=Number(l.maxRaiseTo),max=serverMax>current?serverMax:Math.max(current,fallbackMax);
  const rawMin=current===0?Number(g.minRaise||1000):current+Number(g.minRaise||1000),serverMin=Number(l.minRaiseTo),min=Math.min(max,serverMin>current?serverMin:rawMin),canRaise=max>current;
  const fallbackAllIn=Math.max(0,max-myRound),serverAllIn=Number(l.allInAmount),allInAmount=serverAllIn>0?serverAllIn:fallbackAllIn;
  const half=Math.min(max,Math.max(min,Math.floor((current+(Number(g.pot||0)+call)/2)/1000)*1000));
  const raise=canRaise?`<div class="seven-raise"><div><button data-seven-target="${half}" type="button">½ POT<small>${money(half)}</small></button><button data-seven-target="${max}" class="max" type="button">ALL-IN<small>${money(allInAmount)}</small></button></div><div><input id="sevenRaiseTo" type="number" min="${min}" max="${max}" step="1000" value="${min}" inputmode="numeric"><button class="primary" data-seven-action="raise" type="button">RAISE</button></div></div>`:'';
  return `<div class="seven-action-bar"><button class="danger" data-seven-action="fold" type="button">FOLD</button>${call?`<button class="secondary" data-seven-action="call" type="button">CALL ${money(Math.min(call,Number(g.stack?.user||0)))}</button>`:`<button class="secondary" data-seven-action="check" type="button">CHECK</button>`}${raise}</div>`;
}
async function startSevenPoker(){const btn=$('#startSevenPoker');if(btn.disabled)return;if(!confirm(`세븐포커는 보유머니 ${money(me.balance)} 전액을 '테이블 칩'으로 옮겨서 플레이해.\n\n⚠️ 시작하자마자 전액이 베팅되는 게 아니야. 실제로 판에 걸리는 금액은 ANTE와 내가 선택한 CALL/RAISE뿐이야.\n\n게임 종료 후 남은 테이블 칩은 다시 보유머니로 정산돼.\n\n입장할까?`))return;btn.disabled=true;try{const d=await api('/api/solo/seven/start',{method:'POST',body:'{}'});me=d.user;updateHeader();renderSevenPoker(d.game)}catch(e){toast(e.message)}finally{btn.disabled=false}}
...
function renderSevenPoker(g){
  ...
  const result=g.result?`<div class="seven-result ${g.result.winner}"><small>SHOWDOWN</small><h3>${html(g.result.text)}</h3><div><span>내 패 <b>${html(g.result.userRank||'—')}</b></span><span>J-BOT <b>${html(g.result.botRank||'—')}</b></span><strong>POT ${money(g.result.pot)}</strong></div><div class="seven-profit"><span>이번 판 손익 <b>${signedMoney(Number(g.result.userNet||0))}</b></span><span>테이블 누적 <b>${signedMoney(Number(g.result.sessionNet||0))}</b></span></div></div>`:'';
  root.innerHTML=`<div class="seven-toolbar panel"><div><small>HAND #${g.handNo} · ${SEVEN_STREET_NAME[g.street]||g.street}</small><h3>${avatarImg(me.avatar,me.nickname,'seven-toolbar-face',me.cosmetics)} ${html(me.nickname)} <i>VS</i> ${botFace('poker','seven-toolbar-face')} J-BOT</h3></div><div class="seven-bank"><span>테이블에 가져온 금액 <b>${money(g.buyIn)}</b></span><span>현재 내 칩 <b>${money(g.stack.user)}</b></span><span>현재 판 POT <b>${money(g.pot)}</b></span><span>J-BOT <b>${money(g.stack.bot)}</b></span><button class="secondary seven-cashout" ${!g.complete?'disabled':''} type="button">정산 후 나가기</button></div></div>${result}<div class="seven-layout"><div class="seven-table-shell panel"><div class="seven-felt"><div class="seven-logo">JUNJA<br><span>SEVEN POKER</span></div><div class="seven-opponent"><div class="seven-player-title">${botFace('poker','seven-face')}<div><b>J-BOT</b><small>${html(g.botVisibleStatus?.name||'공개패 분석')}</small></div></div>${sevenCardStrip(g.cards.bot,'bot')}</div><div class="seven-pot"><small>현재 판에 쌓인 칩 · TOTAL POT</small><b>${money(g.pot)}</b><div class="seven-pot-breakdown"><span>내 투입 <strong>${money(Math.max(0,Number(g.buyIn||0)-Number(g.stack?.user||0)))}</strong></span><span>현재 내 칩 <strong>${money(g.stack.user)}</strong></span></div><span class="seven-last-action">${html(g.lastAction||'')}</span></div><div class="seven-me"><div class="seven-player-title">${avatarImg(me.avatar,me.nickname,'seven-face',me.cosmetics)}<div><b>${html(me.nickname)} · 나</b><small>${g.turn==='user'&&!g.complete?'🔥 YOUR TURN':'TABLE PLAYER'}</small></div></div>${sevenCardStrip(g.cards.user,'user')}</div></div>${sevenActions(g)}</div><aside class="seven-side"><div class="panel street-panel"><small>HAND PROGRESS</small>${[3,4,5,6,7].map(n=>`<div class="${g.street===n?'active':g.street>n?'done':''}"><b>${n}</b><span>${SEVEN_STREET_NAME[n]}</span></div>`).join('')}</div><div class="panel">${sevenHandImpact(g.userStatus)}</div><div class="panel seven-rules-mini"><small>7 CARD STUD</small><p>4·5·6번째 카드는 공개, 마지막 7번째 카드는 비공개. 최종 7장 중 최고의 5장으로 승부해.</p><label class="auto-next-toggle mini"><input id="sevenAutoNextTable" type="checkbox" ${storageGet('seven_auto')==='1'?'checked':''}><span></span><b>AUTO NEXT HAND</b></label></div></aside></div>`;
  ...
  root.querySelectorAll('[data-seven-target]').forEach(b=>b.onclick=e=>{e.preventDefault();const input=$('#sevenRaiseTo',root);if(!input)return;const target=Math.max(Number(input.min||0),Math.min(Number(input.max||Number.MAX_SAFE_INTEGER),Number(b.dataset.sevenTarget||0)));input.value=String(target);input.valueAsNumber=target;input.setAttribute('value',String(target));input.dispatchEvent(new Event('input',{bubbles:true}));fx()});
...
function sevenMultiActions(room,s){
  if(s.complete)return `<div class="seven-action-bar complete"><span>핸드 종료 · 다시 하려면 참가자 모두 READY 후 방장이 게임 시작</span></div>`;
  if(!room.myTurn)return `<div class="seven-action-bar waiting"><span>⏳ ${html(room.turnNickname||'상대')} 액션 대기 중...</span></div>`;
  const l=s.legal||{},mine=room.players.find(p=>Number(p.userId)===Number(me.id)),myRound=Number(s.roundBet?.[me.id]??s.roundBet?.[String(me.id)]??0),current=Number(s.currentBet||0);
  const oppCovers=room.players.filter(p=>Number(p.userId)!==Number(me.id)&&!s.folded?.[p.userId]&&!s.folded?.[String(p.userId)]).map(p=>Number(s.roundBet?.[p.userId]??s.roundBet?.[String(p.userId)]??0)+Number(p.stack||0));
  const fallbackMax=Math.min(myRound+Number(mine?.stack||0),oppCovers.length?Math.max(...oppCovers):myRound+Number(mine?.stack||0)),serverMax=Number(l.maxRaiseTo),max=serverMax>current?serverMax:Math.max(current,fallbackMax);
  const call=Math.max(0,Number(l.toCall||0)),rawMin=current===0?Number(s.minRaise||1000):current+Number(s.minRaise||1000),serverMin=Number(l.minRaiseTo),min=Math.min(max,serverMin>current?serverMin:rawMin),canRaise=max>current;
  const fallbackAllIn=Math.max(0,max-myRound),serverAllIn=Number(l.allInAmount),allInAmount=serverAllIn>0?serverAllIn:fallbackAllIn;
  const half=Math.min(max,Math.max(min,Math.floor((current+(Number(s.pot||0)+call)/2)/1000)*1000));
  const raise=canRaise?`<div class="seven-raise"><div><button data-seven-mtarget="${half}" type="button">½ POT<small>${money(half)}</small></button><button data-seven-mtarget="${max}" class="max" type="button">ALL-IN<small>${money(allInAmount)}</small></button></div><div><input id="sevenMultiRaise" type="number" min="${min}" max="${max}" step="1000" value="${min}" inputmode="numeric"><button class="primary" data-seven-multi="raise" type="button">RAISE</button></div></div>`:'';
  return `<div class="seven-action-bar"><button class="danger" data-seven-multi="fold">FOLD</button>${call?`<button class="secondary" data-seven-multi="call">CALL ${money(Math.min(call,Number(mine?.stack||0)))}</button>`:'<button class="secondary" data-seven-multi="check">CHECK</button>'}${raise}</div>`;
}
function renderSevenPokerMulti(room){$('#sevenpokerBrowser').classList.add('hidden');const root=$('#sevenpokerRoom');root.classList.remove('hidden');const s=room.seven;if(!s){root.innerHTML=roomToolbar(room)+`<div class="seven-wait-stage panel"><div class="seven-neon">SEVEN</div><b>LIVE 7 CARD STUD · 2~6인</b><span>2명 이상 입장 후 참가자 모두 READY 하면 방장이 시작할 수 있어.</span><small>봇 좌석 기능은 다음 단계에서 이 대기실에 추가돼.</small></div>`;bindRoomCommon(root,room);return}const opponents=room.players.filter(p=>Number(p.userId)!==Number(me.id)),opp=opponents[0],mine=room.players.find(p=>Number(p.userId)===Number(me.id)),myCards=s.cards?.[me.id]||s.cards?.[String(me.id)]||[],oppCards=s.cards?.[opp?.userId]||s.cards?.[String(opp?.userId)]||[],myStatus=s.statuses?.[me.id]||s.statuses?.[String(me.id)]||{},oppStatus=s.statuses?.[opp?.userId]||s.statuses?.[String(opp?.userId)]||{};let result='';if(s.complete&&s.result){const winners=s.result.winnerIds||[s.result.winnerId].filter(Boolean),win=winners.map(Number).includes(Number(me.id)),handNet=Number(s.result.netByUser?.[me.id]??s.result.netByUser?.[String(me.id)]??0),sessionNet=Number(s.sessionNet??0);result=`<div class="seven-result ${win?'user':winners.length?'bot':'tie'}"><small>SHOWDOWN</small><h3>${html(s.result.summary||'핸드 종료')}</h3><div><span>내 패 <b>${html(s.result.ranks?.[me.id]?.name||s.result.ranks?.[String(me.id)]?.name||myStatus.name||'—')}</b></span><span>상대 <b>${html(s.result.ranks?.[opp?.userId]?.name||s.result.ranks?.[String(opp?.userId)]?.name||oppStatus.name||'—')}</b></span><strong>POT ${money(s.result.pot||s.pot)}</strong></div><div class="seven-profit"><span>이번 판 손익 <b>${signedMoney(handNet)}</b></span><span>테이블 누적 <b>${signedMoney(sessionNet)}</b></span></div></div>`}root.innerHTML=`${roomToolbar(room)}${result}<div class="seven-layout"><div class="seven-table-shell panel"><div class="seven-felt"><div class="seven-logo">JUNJA<br><span>LIVE SEVEN</span></div><div class="seven-opponents-grid">${opponents.map(p=>{const pc=s.cards?.[p.userId]||s.cards?.[String(p.userId)]||[],ps=s.statuses?.[p.userId]||s.statuses?.[String(p.userId)]||{};return `<div class="seven-opponent"><div class="seven-player-title">${avatarImg(p.avatar,p.nickname,'seven-face',p.cosmetics)}<div><b>${html(p.nickname||'상대')}</b><small>${html(ps.name||'공개패 분석')}</small></div><strong>${money(p.stack||0)}</strong>${reactionBubble(room,p.userId)}</div>${sevenCardStrip(pc,'bot')}</div>`}).join('')}</div><div class="seven-pot"><small>HAND #${s.handNo} · ${SEVEN_STREET_NAME[s.street]||s.street}</small><b>${money(s.pot)}</b><span>${html(s.lastAction||'')}</span></div><div class="seven-me"><div class="seven-player-title">${avatarImg(me.avatar,me.nickname,'seven-face',me.cosmetics)}<div><b>${html(me.nickname)} · 나</b><small>${room.myTurn&&!s.complete?'🔥 YOUR TURN':'TABLE PLAYER'}</small></div><strong>${money(mine?.stack||0)}</strong>${reactionBubble(room,me.id)}</div>${sevenCardStrip(myCards,'user')}</div></div>${sevenMultiActions(room,s)}</div><aside class="seven-side"><div class="panel street-panel"><small>HAND PROGRESS</small>${[3,4,5,6,7].map(n=>`<div class="${s.street===n?'active':s.street>n?'done':''}"><b>${n}</b><span>${SEVEN_STREET_NAME[n]}</span></div>`).join('')}</div><div class="panel">${sevenHandImpact(myStatus)}</div><div class="panel seven-rules-mini"><small>LIVE 7 CARD STUD</small><p>다운 2장 + 오픈 1장으로 시작. 4·5·6번째 공개, 7번째 비공개. 최고의 5장 족보로 승부.</p></div></aside></div>`;bindRoomCommon(root,room);root.querySelectorAll('[data-seven-mtarget]').forEach(b=>b.onclick=e=>{e.preventDefault();const i=$('#sevenMultiRaise',root);if(!i)return;const target=Math.max(Number(i.min||0),Math.min(Number(i.max||Number.MAX_SAFE_INTEGER),Number(b.dataset.sevenMtarget||0)));i.value=String(target);i.valueAsNumber=target;i.setAttribute('value',String(target));i.dispatchEvent(new Event('input',{bubbles:true}))});$$('[data-seven-multi]',root).forEach(b=>b.onclick=async()=>{try{b.disabled=true;const d=await api(`/api/rooms/${room.id}/seven/action`,{method:'POST',body:JSON.stringify({action:b.dataset.sevenMulti,raiseTo:Number($('#sevenMultiRaise',root)?.value||0)})});renderRoom(d.room);if(d.room.seven?.complete)await refreshMe()}catch(e){toast(e.message);b.disabled=false}})}
...
Object.assign(window,{go,refreshMe,avatarImg,toast,api});
boot();
