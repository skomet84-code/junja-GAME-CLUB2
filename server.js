...
function gostopBotTurn(userId){const s=soloGostop.get(userId);if(!s||s.phase!=='playing')return;s.bombSkips=s.bombSkips||{user:0,bot:0};if(s.bombSkips.bot>0){s.bombSkips.bot--;if(s.deck.length)gostopDrawAndCapture(s,'bot',{playedMonth:null,playMatched:false});matgoEvent(s,'💣 폭탄 보정 · J-BOT이 더미패 1장 뒤집음','gold');gostopAfterTurn(userId,'bot');if(s.phase==='playing'&&!s.needDecision)s.turn='user';return;}let best=0,bestScore=-1;s.hands.bot.forEach((c,i)=>{const matches=s.floor.filter(f=>f.m===c.m).length;const score=matches*30+({광:8,'열끗':5,'띠':3,'피':1}[c.type]||0)+(c.bird||c.ribbon?4:0);if(score>bestScore){bestScore=score;best=i}});const card=s.hands.bot.splice(best,1)[0],matches=s.floor.filter(c=>c.m===card.m);const choice=matches.length===2?[...matches].sort((a,b)=>({광:4,'열끗':3,'띠':2,'피':1}[b.type]-({광:4,'열끗':3,'띠':2,'피':1}[a.type])))[0].id:null;const played=matgoCapture(s,'bot',card,choice);gostopDrawAndCapture(s,'bot',{playedMonth:card.m,playMatched:played.matched});gostopAfterTurn(userId,'bot');if(s.phase==='playing'&&!s.needDecision)s.turn='user';}
...
function roomGostopPlay(r,userId,cardId,choiceId=null,action=null,month=null){
 const s=r.gostop;if(!s||s.phase!=='playing')throw new Error('진행 중인 맞고가 없습니다.');const side=roomGostopSide(s,userId);if(s.turn!==side||s.needDecision)throw new Error('지금은 내 차례가 아닙니다.');s.bombSkips=s.bombSkips||{user:0,bot:0};if(s.bombSkips[side]>0){s.bombSkips[side]--;if(s.deck.length)gostopDrawAndCapture(s,side,{playedMonth:null,playMatched:false});matgoEvent(s,'💣 폭탄 보정 · 더미패 1장 뒤집음','gold');roomGostopAfterTurn(r,side);return}
