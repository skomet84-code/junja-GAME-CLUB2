...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(url.pathname==='/api/slot/spin'&&req.method==='POST'){
      const u=requireAuth(req,res);if(!u)return;const b=await readBody(req);let bet;try{bet=gameWager(b.bet,1000,1000000000,1000)}catch(e){return json(res,400,{error:e.message})};
